...
function layoutTiles(mode,animate=false,origins=null){
 ...
 rows.forEach(x=>{
  ...
  if(p&&animate&&!reduce&&from?.width)b.animate([{transform:`translate(${(from.left-box.left)/uiScale-p.x}px,${(from.top-box.top)/uiScale-p.y}px)`},{transform:'translate(0,0)'}],{duration:1050,delay:mode==='groups'?120+methods.findIndex(m=>m.id===x.method)*26:0,easing:'cubic-bezier(.22,1,.36,1)'});
 });
...
function renderQuestion(){
 ...
 if(s<5){
  ...
  const budgetOptions=[20,50,100,null];if(!budgetOptions.includes(state.budget))budgetOptions.splice(3,0,state.budget);
  $('#question').innerHTML=`<div class="result-controls"><div><label>Time budget</label><div class="quick-budgets">${budgetOptions.map(v=>`<button type="button" data-budget="${v}" class="${state.budget===v?'chosen':''}" aria-pressed="${state.budget===v}">${v===null?'No limit':v+' ms'}</button>`).join('')}</div></div><label>Pose threshold<select id="angle-filter">${[5,10,20].map(v=>`<option value="${v}">${v}°</option>`).join('')}</select></label><label>Sort within matcher<select id="sort"><option value="method">Resolution · precision</option><option value="accuracy">Highest pose AUC</option><option value="runtime">Lowest runtime</option></select></label><button class="text-button" id="edit">Change requirements</button><details class="more-filters"><summary>More filters</summary><div class="filter-fields"><label>Time limit · ms<input id="runtime-filter" type="number" min="0" step="any" placeholder="No limit"></label><label>GPU memory limit · GiB<input id="memory-filter" type="number" min="0" step="any" placeholder="No limit"></label><label>Resolution<select id="res-filter"><option value="">All resolutions</option>${[128,256,512,1024].map(r=>`<option value="${r}">${r} px</option>`).join('')}</select></label><label>Precision<select id="precision-filter"><option value="">All supported modes</option>${['native','fp32','mp','fp16'].map(v=>`<option value="${v}">${v.toUpperCase()}</option>`).join('')}</select></label><label>Energy limit · J / pair<input id="energy-filter" type="number" min="0" step="0.1" placeholder="No limit"></label><button id="apply-filters" class="primary">Apply</button><a href="measurement.html#energy">Power measurement scope</a></div></details></div>`;
  $('#runtime-filter').value=state.budget??'';$('#memory-filter').value=state.memory??'';
  $('#res-filter').value=state.resolution??'';$('#precision-filter').value=state.precision??'';$('#energy-filter').value=state.energy??'';
  $('#apply-filters').onclick=()=>{const inputs=[$('#runtime-filter'),$('#memory-filter'),$('#energy-filter')];if(!inputs.every(input=>input.reportValidity()))return;state.budget=inputs[0].value===''?null:inputs[0].valueAsNumber;state.memory=inputs[1].value===''?null:inputs[1].valueAsNumber;state.energy=inputs[2].value===''?null:inputs[2].valueAsNumber;state.resolution=Number($('#res-filter').value)||null;state.precision=$('#precision-filter').value||null;go(5);};
  ...
  const fold=document.createElement('details');fold.className='adjust-requirements';fold.open=true;const label=document.createElement('summary');label.textContent='Adjust requirements';fold.append(label,...$('#question').children);$('#question').replaceChildren(fold);
 }
}
...
function selectConfiguration(key){
 ...
 if(window.Motion&&!reducedMotion())motionControls.push(Motion.animate('#inspect',{opacity:[0,1]},{duration:.3}));
}
