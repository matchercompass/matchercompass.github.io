...
let state={step:0,task:tasks[0][0],platform:'thor',budget:50,memory:null,energy:null,resolution:null,precision:null,angle:10,sort:'method',selected:null};
...
function rejection(x){
 if(state.step>=1&&(x.oom||x.runtime===null||x.memory===null))return x.oom?'Out of memory during resource measurement.':'Required measurements are unavailable.';
 if(state.step>=1&&state.resolution!==null&&x.res!==state.resolution)return 'Outside the selected resolution.';
 if(state.step>=1&&state.precision!==null&&x.precision!==state.precision)return 'Outside the selected precision.';
 ...
 if(state.step>=2&&state.budget!==null&&x.runtime>state.budget)return `Runtime of ${fmt(x.runtime)} ms exceeds the ${state.budget} ms limit.`;
 if(state.step>=3&&state.memory!==null&&x.memory>state.memory)return `GPU memory of ${fmt(x.memory,2)} GiB exceeds the ${state.memory} GiB limit.`;
 return null;
}
...
function makeTile(x){const m=methods.find(m=>m.id===x.method);const b=document.createElement('button');b.type='button';b.className='tile';b.style.setProperty('--color',m.color);b.dataset.key=id(x);b.innerHTML='';b.addEventListener('click',()=>{state.selected=id(x);renderSelection();});nodes.set(id(x),b);return b;}
...
function renderQuestion(){
 ...
 if(s===0){h='Visual condition';desc='';opts=`<div class="options">${tasks.map(x=>option(x[0],x[1],x[2],state.task)).join('')}</div>`;hint='';}
 if(s===1){h='Hardware platform';desc='';opts=`<div class="options">${platforms.map(x=>option(x[0],x[1],x[2],state.platform)).join('')}</div>`;hint='';}
 if(s===2){h='Runtime limit';desc='Median matching time per image pair.';opts=`<div class="button-grid">${[10,20,50,100,null].map(x=>option(x,x===null?'No limit':`${x} ms`,'',state.budget)).join('')}</div>`;hint='Excludes model loading and pose estimation.';}
 if(s===3){h='GPU memory limit';desc='Median peak GPU memory, including the model.';opts=`<div class="button-grid">${[0.5,1,2,null].map(x=>option(x,x===null?'No limit':`${x} GiB`,'',state.memory)).join('')}</div>`;hint='';}
...
function update(){
 ...
 $('#summary').innerHTML=[`${task[1]} · ${task[2]}`,state.step>=1?platform[1]:null,state.step>=2?(state.budget===null?'No runtime limit':`≤ ${state.budget} ms`):null,state.step>=3?(state.memory===null?'No memory limit':`≤ ${state.memory} GiB`):null,state.step>=4?`Pose AUC @ ${state.angle}°`:null,state.resolution?`${state.resolution} px`:null,state.precision?state.precision.toUpperCase():null,state.energy!==null?`≤ ${state.energy} J`:null].filter(Boolean).map(x=>`<span class="chip">${x}</span>`).join('');
...
let resizeFrame;new ResizeObserver(()=>{cancelAnimationFrame(resizeFrame);resizeFrame=requestAnimationFrame(()=>layoutTiles(state.step===5?'groups':'rows'));}).observe($('#board'));
